...
const RecentlyViewedPage = () => {
  ...
  // Pagination logic (update to use filteredProducts)
  ...
  axios.get('/recentlyViewed', {
      ...
      .catch(() => setError('Failed to fetch recently viewed products'))
      .finally(() => setLoading(false));
  }, [token]);

  if (loading) return <div className="flex justify-center items-center min-h-64">Loading recently viewed products...</div>;
  if (error) return <div className="text-red-600 text-center py-8">{error}</div>;
  if (!products.length) return <div className="text-center py-16 text-primary-300 text-lg bg-primary-50 rounded-2xl shadow-card-haven max-w-xl mx-auto">No recently viewed products found.</div>;
    ...
    <div className="container mx-auto py-8 px-4">
      ...
      {totalPages > 1 && (
        <div className="flex justify-center items-center gap-2 mt-8">
          <button
            className="px-3 py-1 rounded bg-pink-100 text-pink-700 font-semibold disabled:opacity-50"
